...
TEAMS: dict[str, TeamMeta] = {
    "KIA": {"code": "KIA", "name": "KIA",  "full_name": "KIA 타이거즈",   "color": "red",          "stadium": "광주-기아 챔피언스 필드", "mascot": "호걸이"},
    ...
    "SSG": {"code": "SSG", "name": "SSG",  "full_name": "SSG 랜더스",      "color": "red3",         "stadium": "인천 SSG 랜더스필드",      "mascot": "랜디"},
...
# 네이버 스포츠 API는 자체 내부 코드(HT/SK)를 사용한다. 사용자가 보는
# 코드(KIA/SSG)와 일치시키기 위해 양방향 매핑을 둔다.
NAVER_ALIAS: dict[str, str] = {
    "HT": "KIA",
    "SK": "SSG",
    "기아": "KIA",
}
...
def normalize(code: str | None) -> str:
    """네이버 내부 코드 또는 한국어 별칭을 정규 팀 코드로 변환."""
    if not code:
        return ""
    up = code.upper().strip()
    return NAVER_ALIAS.get(up, up)
...
def team(code: str | None) -> TeamMeta:
    ...
    canon = normalize(code)
    return TEAMS.get(canon, {
